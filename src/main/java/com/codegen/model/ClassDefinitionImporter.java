...
//		importFromExcel("/Users/kbethalam/Downloads/DBSchema.xlsx", 0);
//	}
	...
	public static List<ClassDefinition> importFromExcel(String filePath,
		...
		try {
			...
			if (sheetNo != -1) {
				ClassDefinition classDefinition = getClassDefinition(workbook,
						sheetNo);
	...
	private static ClassDefinition getClassDefinition(XSSFWorkbook workbook,
			int sheetIndex) {
		XSSFSheet sheet = workbook.getSheetAt(sheetIndex);

		String className = getClassFromSheet(sheet);
		System.out.println("Retrieving definition for :" + className);
		...
		ClassDefinition classDefinition = new ClassDefinition(className,
				tableName, columnDefinitions, dtoDefinition);
	...
	private static int getEndRowWithData(XSSFSheet sheet, int startRow,
			int column) {
	...
	private static List<ColumnDefinition> getColumnDefinitions(XSSFSheet sheet) {

		List<ColumnDefinition> columnDefinitions = new ArrayList<ColumnDefinition>();

		int endRow = getEndRowWithData(sheet, DATA_START_ROW,
				COLUMN_DATA_START_COLUMN);
		if (endRow == -1) {
		...
		for (int currentRow = DATA_START_ROW; currentRow <= endRow; currentRow++) {
			...
			String dbIdentifier = classRow.getCell(2).getStringCellValue()
					.toUpperCase();
			String dbType = classRow.getCell(3).getStringCellValue();
			boolean isForeignKey = false;
			if (classRow.getCell(4) != null){
				isForeignKey = classRow.getCell(4).getBooleanCellValue();
			}
			...
			if (classRow.getCell(6) != null){
				isNotNull = classRow.getCell(6).getBooleanCellValue();
			}
			boolean isUnique = false; 
			if (classRow.getCell(7) != null){
				isUnique = classRow.getCell(7).getBooleanCellValue();
			}

			ColumnDefinition columnDefinition = new ColumnDefinition(fieldId,
					fieldTyp, dbIdentifier, dbType, isForeignKey,
					referenceTable, isNotNull, isUnique);
	...
	private static DTODefinition getDTODefinition(XSSFSheet sheet) {
		...
		boolean isCustomQuerySpecified = getById != null
				&& getById.trim().length() > 0;
		DTODefinition definition = new DTODefinition(columns,
				isCustomQuerySpecified, getById, getAllQuery);
	...
	private static List<DTOColumnDefinition> getDTOColumnDefinitions(
		...
		for (int currentRow = DATA_START_ROW; currentRow <= endRow; currentRow++) {
			...
			String fieldId = classRow.getCell(DTO_COLUMN_DATA_START_COLUMN)
					.getStringCellValue();
			String fieldType = classRow.getCell(
					DTO_COLUMN_DATA_START_COLUMN + 1).getStringCellValue();

			String referenceTable = "";
			if (classRow.getCell(DTO_COLUMN_DATA_START_COLUMN + 2) != null) {
				referenceTable = classRow.getCell(
						DTO_COLUMN_DATA_START_COLUMN + 2).getStringCellValue();
			}

			String referenceTitleField = "";
			if (classRow.getCell(DTO_COLUMN_DATA_START_COLUMN + 3) != null) {
				referenceTitleField = classRow.getCell(
						DTO_COLUMN_DATA_START_COLUMN + 3).getStringCellValue();
			}

			String referingField = "";
			if (classRow.getCell(DTO_COLUMN_DATA_START_COLUMN + 4) != null) {
				referingField = classRow.getCell(
						DTO_COLUMN_DATA_START_COLUMN + 4).getStringCellValue();
			}

			DTOColumnDefinition columnDefinition = new DTOColumnDefinition(
					fieldId, fieldType, referenceTable, referingField,
					referenceTitleField);
